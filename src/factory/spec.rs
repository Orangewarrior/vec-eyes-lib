...
use crate::nlp::{ExternalEmbeddings, FastTextEmbeddings, NlpOption};

/// Entry point for the typed classifier factory.
pub struct ClassifierSpec;

impl ClassifierSpec {
    pub fn bayes() -> BayesSpec {
        BayesSpec::new()
    }
    pub fn knn_cosine(k: usize) -> KnnSpec {
        KnnSpec::new(DistanceMetric::Cosine, k, None)
    }
    pub fn knn_euclidean(k: usize) -> KnnSpec {
        KnnSpec::new(DistanceMetric::Euclidean, k, None)
    }
    pub fn knn_manhattan(k: usize) -> KnnSpec {
        KnnSpec::new(DistanceMetric::Manhattan, k, None)
    }
    pub fn knn_minkowski(k: usize, p: f32) -> KnnSpec {
        KnnSpec::new(DistanceMetric::Minkowski(p), k, Some(p))
    }
...
impl TrainingData {
    ...
    fn resolve(self) -> Result<Vec<TrainingSample>, VecEyesError> {
        ...
        let hot = self.hot_path.ok_or_else(|| {
            VecEyesError::invalid_config("ClassifierSpec", "hot_path or samples() is required")
        })?;
        let cold = self.cold_path.ok_or_else(|| {
            VecEyesError::invalid_config("ClassifierSpec", "cold_path or samples() is required")
        })?;
...
impl BayesSpec {
    ...
    pub fn hot_label(mut self, l: ClassificationLabel) -> Self {
        self.data.hot_label = l;
        self
    }
    pub fn cold_label(mut self, l: ClassificationLabel) -> Self {
        self.data.cold_label = l;
        self
    }
    pub fn recursive(mut self, r: bool) -> Self {
        self.data.recursive = r;
        self
    }
    pub fn samples(mut self, s: Vec<TrainingSample>) -> Self {
        self.data.samples = Some(s);
        self
    }
...
pub struct KnnSpec {
    ...
    normalize: bool,
    data: TrainingData,
    external_embeddings: Option<ExternalEmbeddings>,
}
...
impl KnnSpec {
    ...
    /// Use pre-loaded external embeddings (fastText or word2vec) as the feature pipeline.
    pub fn external_embeddings(mut self, embeddings: ExternalEmbeddings) -> Self {
        self.external_embeddings = Some(embeddings);
        self
    }

    /// Convenience wrapper — equivalent to `.external_embeddings(ExternalEmbeddings::FastText(e))`.
    pub fn external_fasttext(mut self, embeddings: FastTextEmbeddings) -> Self {
        self.external_embeddings = Some(ExternalEmbeddings::FastText(embeddings));
        self
    }
    ...
    pub fn build(self) -> Result<KnnClassifier, VecEyesError> {
        ...
        if let Some(emb) = self.external_embeddings {
            return KnnClassifier::train_with_external_embeddings(
                &samples,
                emb,
                self.metric,
                self.k,
...
impl AdvancedSpec {
    ...
    pub fn threads(mut self, t: usize) -> Self {
        self.config.threads = Some(t);
        self
    }
    pub fn embedding_dims(mut self, d: usize) -> Self {
        self.config.embedding_dimensions = Some(d.max(1));
        self
    }
    pub fn hot_label(mut self, l: ClassificationLabel) -> Self {
        self.data.hot_label = l;
    ...
    pub fn build(self) -> Result<AdvancedClassifier, VecEyesError> {
        ...
        let samples = self.data.resolve()?;
        if let Some(emb) = self.external_embeddings {
            return AdvancedClassifier::train_with_external_embeddings(
                self.method,
                &samples,
                emb,
                hot_label,
