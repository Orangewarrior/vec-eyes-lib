use vec_eyes_lib::{Builder, ClassifierMethod, ClassificationLabel, NlpOption, TypedClassifierBuilder};
...
fn common_builder_trait_is_available_for_pipeline_and_engine() {
    let pipeline = <vec_eyes_lib::NlpPipelineBuilder as Builder<vec_eyes_lib::NlpPipeline>>::new()
        ...
        .build()
        .unwrap();
    assert!(matches!(pipeline.representation, vec_eyes_lib::RepresentationKind::Word2Vec));

    let _engine_builder = <vec_eyes_lib::EngineBuilder as Builder<vec_eyes_lib::compat::Engine>>::new();
}
